...
const Page = async ({ params }) => {
  let emojiForAcc,session,userAcc, posts, rankingOfUser, accType,bansOfUser,isBanned,allRoles,allSub,sessionsOfUser,ipsOfUser,ipsWithOtherUsers
  try{
...
console.log(
  JSON.stringify(ipsWithOtherUsers, null, 2) // Pretty print with 2 spaces of indentation
);
...
{(session?.role?.privileges > 2 &&  userAcc?.id != session.userId && accType?.priority <= 1 || session?.role?.privileges > 3 &&accType?.priority <= 1||  session?.role?.privileges > 3 &&accType?.priority == null )&&
