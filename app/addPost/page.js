...
const Page = async () => {
    ...
    try {
        ...
        await prisma.$disconnect(); // Uzavřete připojení po načtení kategorií
    }
    ...
    try {
        ...
        await prisma.$disconnect(); // Uzavřete připojení po načtení sekcí
    }

    let userCategories;
    try {
        userCategories = await prisma.userCategories.findMany({
            where: {
        ...
        throw new Error("Chyba při načítání uživatelských kategorií: " + dbError.message);
    } finally {
        await prisma.$disconnect(); // Uzavřete připojení po načtení uživatelských kategorií
    }
