...
import { ReportsPostModal , openReportsPostModal, closeReportsPostModal} from  "@/app/components/modals/ReportsPostModal";
...
<div className="flex space-x-4 mt-4 mb-4">
  ...
  <a 
    className="btn border-dotted border-yellow-500 border-2 flex-shrink hover:border-yellow-500" 
    onClick={openReportsPostModal}
  >
      ...
      {postRecord?.userId === session?.userId || 
       session?.role?.privileges > postRecord?.user?.role?.privileges ? (
        <>
          ...
          {session?.role?.privileges > postRecord?.user?.role?.privileges && (
            <>
            <ReportsPostModal postId={postRecord.id} />
...
Ohodnotit uživatele</a>
    ...
    </div> : <> <div style={{textAlign: "center"}} className="flex items-center space-x-2 "> <a className="btn sm:h-0 h-20   flex-shrink" onClick={openEditPostModal}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6 text-yellow-600">
