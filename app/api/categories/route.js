...
export async function PUT(req) {
    try {
        ...
        await updateUserCategory(data, session); // Ensure to await this function
      
        return new Response(
            JSON.stringify("Úspěšně upravena kategorie uživatelem"),
            {
                status: 200,
                headers: { 'Content-Type': 'application/json' }
            }
        ...
        console.error('Chyba při editování kategorií:', error);
        return new Response(
            JSON.stringify({ message: 'Chyba na serveru [PUT] editování kategorií' }),
            {
                status: 500,
                headers: { 'Content-Type': 'application/json' }
            }
