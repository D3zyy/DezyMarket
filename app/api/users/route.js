...
export async function POST(req) {
  try {
    ...
    if (user) {
      ...
      if (ban) {
        messageBan = ban.pernament 
          ? (ban.reason == null 
              ? `Váš účet byl trvale zablokován. Pokud si myslíte, že došlo k omylu, kontaktujte nás prosím.`
              : `Váš účet byl trvale zablokován z důvodu: ${ban.reason}. Pokud si myslíte, že došlo k omylu, kontaktujte nás prosím.`)
          : (ban.reason == null 
              ? `Účet byl zablokován do: ${ban.banTill}. Pokud si myslíte, že došlo k omylu, kontaktujte nás prosím.`
              : `Účet byl zablokován do: ${ban.banTill} z důvodu: ${ban.reason}.  Pokud si myslíte, že došlo k omylu, kontaktujte nás prosím.`);
      }
    }
    ...
    if (user && isPasswordValid && !ban) {
      ...
      // Check user's number of login attempts
      if (user.firstLogin) {
