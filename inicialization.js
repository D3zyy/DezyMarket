...
export async function POST(request) {
    ...
    try {
        ...
        if (event.type === 'invoice.payment_succeeded') {
            ...
            const fromDate = DateTime.fromSeconds(currentPeriodStart)
                .setZone('Europe/Prague')
                .toFormat("yyyy-MM-dd'T'HH:mm:ss'+00:00'");
            ...
            const toDate = DateTime.fromSeconds(currentPeriodStart)
                .setZone('Europe/Prague')
                .plus({ days: diffDays })  // Připočítáme dny k fromDate
                .toFormat("yyyy-MM-dd'T'HH:mm:ss'+00:00'");

            const nextPaymentDate = DateTime.fromSeconds(currentPeriodEnd)
                .setZone('Europe/Prague')
                .plus({ days: 1 }) // Předpokládáme, že příští platba je po jednom dni, ale může to být upraveno
                .toFormat("yyyy-MM-dd'T'HH:mm:ss'+00:00'");
            ...
            if (paymentIntent.billing_reason === "subscription_create") {
                ...
                const updatedToDate = DateTime.fromSeconds(currentPeriodEnd)
                    .setZone('Europe/Prague')
                    .toFormat("yyyy-MM-dd'T'HH:mm:ss'+00:00'");

                const updated = await prisma.AccountTypeUsers.updateMany({
                    where: {
                        userId: user.id,
                ...
                if (updated.count === 0) {
                    throw new Error(`Nepodařilo se aktualizovat předplatné pro uživatele ${user.id}.`);
                }
